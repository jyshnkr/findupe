"""Aggregation + plain-text rendering for `dupefinder stats`/`history`.
...
from .ledger import SCANS_DIR, ScanRecord, list_scans
from .report import _fmt_bytes
from .trash import UNDO_DIR, list_manifests
...
def aggregate_undo_totals(undo_dir: Path = UNDO_DIR, scans_dir: Path = SCANS_DIR) -> Totals:
...
def applied_scan_ids(undo_dir: Path = UNDO_DIR) -> set[str]:
...
def reclaimed_timeline(undo_dir: Path = UNDO_DIR) -> list[tuple[str, int]]:
...
def render_stats_text(records: list[ScanRecord], totals: Totals, applied_ids: set[str]) -> str:
    lines = [
        ...
        f"{_fmt_bytes(totals.bytes_reclaimed_net)} reclaimed (net of restores)",
        f"{totals.files_restored} restored · {totals.files_failed} failed",
        f"{totals.duplicates_found_total} surplus files flagged across "
        f"{totals.scans_recorded} scans (not deduplicated — the same "
        f"unresolved duplicate found again in a later scan counts again)",
    ]
    return "\n".join(lines)
